...
const Header = () => {
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 z-50 h-[${headerHeight}rem] transition-all duration-300 ${isScrolled ? 'bg-black/80 backdrop-blur-sm' : ''}`}
            style={{
        ...
        >
            {!isScrolled && <div className="h-2 md:h-5 w-full">
                </div>}
            <div className="container pl-[1.5rem] pr-[0.6rem] h-full">
                <div className="header-container justify-center my-auto h-full">
                    <div className="flex flex-row items-center justify-between content-center h-full">
                        <Link href={"/"} className={`transition-all  duration-300 flex align-center`}
                        >
                            <img
                                src={'/images/logo.png'}
                                alt="logo image"
                                className={`transition-all duration-300`}
                                style={{ maxHeight: `${headerHeight - 1}rem` }}
                            />
                        </Link>
